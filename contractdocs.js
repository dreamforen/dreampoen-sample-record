...
(function dfContractDocuments(){
  ...
  async function deletePackage(id){
    if(!id||!confirm('이 계약문서 세트를 삭제할까요?\n기존 계약관리·업체현황 자료에는 영향이 없습니다.'))return;const client=db();if(!client)return alert('DB 연결을 확인해주세요.');
    const {error}=await client.from(TABLE).delete().eq('id',id);if(error)return alert('삭제하지 못했습니다.\n'+error.message);closeEditor();loaded=false;await loadPackages(true);setMessage('계약문서 세트를 삭제했습니다.','ok');
  }
  ...
  function inline(x,y,w,text){return layer(x-1,y-1,w+2,14,text,{size:8.5,min:5.1,align:'center'})}
  ...
  function renderContract(d){
    ...
    layers.push(layer(318,112,40,15,d.receipt_no,{mask:false,size:7.5,align:'center'}),layer(389,112,44,15,dateText(d.receipt_date)==='-'?'':dateText(d.receipt_date),{mask:false,size:6.8,align:'center'}),layer(180,138,100,16,d.management_agency,{mask:false,size:7.5,align:'center'}));
    layers.push(layer(119,189,160,16,d.client_name,{mask:false,size:8.5}),layer(151,207,128,34,d.client_address,{mask:false,size:7.3,top:true}),layer(151,228,128,15,d.client_representative,{mask:false,size:8}),layer(168,247,111,15,d.client_biz_no,{mask:false,size:7.5}),layer(88,282,191,20,d.client_phone,{mask:false,size:8,align:'center'}));
    layers.push(layer(119,314,411,17,d.service_name,{mask:false,size:9}),layer(121,334,24,17,c.y,{mask:false,size:8,align:'right'}),layer(160,334,25,17,c.m,{mask:false,size:8,align:'right'}),layer(200,334,25,17,c.d,{mask:false,size:8,align:'right'}),layer(401,334,25,17,d.duration_months,{mask:false,size:8,align:'center'}));
    layers.push(layer(121,354,24,17,s.y,{mask:false,size:8,align:'right'}),layer(160,354,25,17,s.m,{mask:false,size:8,align:'right'}),layer(200,354,25,17,s.d,{mask:false,size:8,align:'right'}),layer(337,354,24,17,e.y,{mask:false,size:8,align:'right'}),layer(376,354,25,17,e.m,{mask:false,size:8,align:'right'}),layer(416,354,20,17,e.d,{mask:false,size:8,align:'right'}));
    const numericAmount=String(d.contract_amount||'').trim()?money(d.contract_amount).toLocaleString('ko-KR'):'';
    layers.push(layer(186,373,344,17,d.site_address,{mask:false,size:8}),layer(147,393,38,17,amountWords(d),{mask:false,size:5.6,min:4.6,align:'center'}),layer(196,393,64,17,numericAmount,{mask:false,size:7,align:'right'}));
    if(d.vat_mode!=='별도')layers.push(layer(269,393,49,17,`*VAT ${d.vat_mode}`,{size:7,align:'center'}));
    layers.push(layer(132,413,398,17,d.contract_guarantee,{mask:false,size:8}),layer(132,432,398,17,d.advance_payment,{mask:false,size:8}),layer(132,452,398,17,d.penalty_rate,{mask:false,size:8}));
    layers.push(layer(369,659,26,17,sign.y,{mask:false,size:8,align:'right'}),layer(429,659,26,17,sign.m,{mask:false,size:8,align:'right'}),layer(479,659,26,17,sign.d,{mask:false,size:8,align:'right'}));
    layers.push(layer(151,698,155,40,d.client_address,{mask:false,size:7.2,top:true}),layer(148,739,48,17,d.client_representative,{mask:false,size:8,align:'center'}));
    return page('standard_contract-1.png',layers,'측정대행표준계약서');
  }
  function renderPlan(d){
    ...
    pages.push(page('task_plan-1.png',[layer(92,238,417,29,`[용역명 : ${year.slice(2)}년 ${n} ${d.service_name}]`,{size:13.2,min:7,align:'center',bold:true}),layer(251,449,94,31,`${year}. ${month}`,{size:17,min:12,align:'center',bold:true})],'과업수행계획서 표지'));
    const p2=[];p2.push(layer(132,123,210,18,d.service_name,{size:8.8,min:6.2}));
    ...
    p2.push(layer(132,297,300,17,d.site_address||d.client_address,{size:8.5,min:5.8}),layer(150,316,181,17,`${period(d.start_date)} ∼ ${period(d.end_date)}`,{size:8.5,min:6.2}),layer(150,374,180,17,n,{size:8.5,min:5.8}));
    const fac=(d.facilities||[]).slice(0,3),join=k=>fac.map(x=>x[k]||'').filter(Boolean).join('\n');
    p2.push(layer(58,439,141,39,join('emission'),{size:6.7,min:5.2,align:'center'}),layer(201,439,128,39,join('prevention'),{size:6.7,min:5.2,align:'center'}),layer(331,439,76,39,join('cycle'),{size:6.7,min:5.2,align:'center'}),layer(409,439,68,39,join('items'),{size:6.7,min:5.2,align:'center'}),layer(479,439,41,39,join('quantity'),{size:6.7,min:5.2,align:'center'}));
    p2.push(layer(132,508,112,17,d.team_text,{size:8.5,min:6}));
    const eq=(d.equipment||[]).slice(0,8);for(let i=0;i<8;i++){const y=569.5+i*22.4,x=eq[i]||{};p2.push(layer(117,y,125,20,x.name||'',{size:7,min:5.2,align:'center'}),layer(243,y,108,20,x.maker||'',{size:7,min:5.2,align:'center'}),layer(352,y,98,20,x.model||'',{size:7,min:5.2,align:'center'}),layer(451,y,69,20,x.note||'',{size:7,min:5.2,align:'center'}))}
    pages.push(page('task_plan-2.png',p2,'과업수행계획서 1쪽'));
    pages.push(page('task_plan-3.png',[inline(395.52,260.88,73.2,n+'의'),inline(190.8,299.28,73.2,withP(n))],'과업수행계획서 2쪽'));
    pages.push(page('task_plan-4.png',[layer(194,97,53,16,fee?money(fee).toLocaleString('ko-KR'):'',{size:8,min:5.8,align:'center'}),inline(176.4,195.12,73.2,n+'의'),inline(86.64,303.12,73.2,n+'의'),inline(286.56,360.72,95.04,n+'로부터'),inline(357.96,456.72,73.2,n+'의'),inline(149.52,475.92,73.2,withP(n)),inline(350.76,475.92,73.2,n+'의'),inline(447.36,533.52,62.28,n),inline(88.2,552.72,73.32,n+'의'),inline(211.56,610.32,73.2,topic(n))],'과업수행계획서 3쪽'));
    pages.push(page('task_plan-5.png',[inline(92.16,99.12,63.94,n+','),inline(142.56,195.12,73.32,n+'의'),inline(474.6,425.52,63.82,n+',')],'과업수행계획서 4쪽'));
    return pages.join('');
  }
  function renderNotice(d){
    ...
    layers.push(layer(109,281,79,39,d.client_name,{size:7,min:4.8,align:'center'}),layer(189,281,94,39,d.site_address||d.client_address,{size:6.5,min:4.6,align:'center'}),layer(284,281,61,39,items,{size:6.3,min:4.5,align:'center'}),layer(346,281,52,39,fee,{size:6.5,min:4.8,align:'center'}),layer(399,281,60,39,dateText(d.start_date)==='-'?'':dateText(d.start_date),{size:6.5,min:5,align:'center'}),layer(460,281,75,39,dateText(d.end_date)==='-'?'':dateText(d.end_date),{size:6.5,min:5,align:'center'}),layer(536,281,29,39,d.notice_remark,{size:6,min:4.5,align:'center'}));
    layers.push(layer(478,369,79,18,sign.y?`${sign.y}년 ${String(sign.m).padStart(2,'0')}월 ${String(sign.d).padStart(2,'0')}일`:'',{size:7.2,min:6,align:'center'}),layer(376,398,40,17,PROVIDER.representative,{size:8,align:'center'}),layer(82,413,80,35,authority,{size:8,min:5.5,align:'center'}));
    return page('contract_notice-1.png',layers,'측정대행계약 체결사실 통보서');
  }

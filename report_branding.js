...
(function(global){
'use strict';
const VERSION='120373400',BASE='assets/report_branding/',MIME='application/hwp+zip',WM_MARK='DF_REPORT_BRANDING_WATERMARK_V1',SEAL_MARK='DF_REPORT_BRANDING_SEAL_V1';
...
async function assets(options={}){const decodeBytes=x=>new TextDecoder().decode(x);const manifest=JSON.parse(decodeBytes(await loadAsset('manifest.json',options)));const [watermark,seal,master,header,sealPicture]=await Promise.all([manifest.watermark.file,manifest.seal.file,manifest.masterpage,manifest.header,manifest.seal_picture].map(n=>loadAsset(n,options)));if(await hash(watermark)!==manifest.watermark.sha256||await hash(seal)!==manifest.seal.sha256)fail('성적서 워터마크·직인 원본이 배포 설정과 다릅니다.');return {manifest,watermark,seal,master:fixedMaster(decodeBytes(master)),header:decodeBytes(header),sealPicture:decodeBytes(sealPicture)};}
...
// Repair existing saved reports as well as newly generated ones. Only a recognized
// company report is normalized; unrelated HWPX files retain the link-only repair.
async function prepareDownload(input,options={}){
 ...
 if((await sectionParts(zip)).some(s=>s.pages.length))return normalize(inputBytes,options);
...
// The reference watermark formerly occupied a 44,526-unit inline line after
// seven empty paragraphs. A floating PAPER object does not consume text flow.
function fixedMaster(xml){
 const tree=parse(xml),pic=desc(tree,'pic')[0],pos=first(pic,'pos'),size=first(pic,'sz'),width=Number(attr(size,'width')),height=Number(attr(size,'height')),edits=[];
 let open=pic.open;for(const [k,v] of Object.entries({textWrap:'BEHIND_TEXT',lock:'1'}))open=setAttr(open,k,v);edits.push({start:pic.start,end:pic.openEnd,text:open});
 let position=pos.open;for(const [k,v] of Object.entries({treatAsChar:'0',affectLSpacing:'0',flowWithText:'0',allowOverlap:'1',holdAnchorAndSO:'1',vertRelTo:'PAPER',horzRelTo:'PAPER',vertAlign:'TOP',horzAlign:'LEFT',vertOffset:18367,horzOffset:11428}))position=setAttr(position,k,v);edits.push(patch(pos,position));
 for(const name of ['orgSz','curSz']){const n=first(pic,name);edits.push(patch(n,setAttr(setAttr(n.open,'width',width),'height',height)));}
 const offset=first(pic,'offset');edits.push(patch(offset,setAttr(setAttr(offset.open,'x','0'),'y','0')));
 const rotation=first(pic,'rotationInfo');edits.push(patch(rotation,setAttr(setAttr(rotation.open,'centerX',Math.floor(width/2)),'centerY',Math.floor(height/2))));
 edits.push(patch(first(pic,'renderingInfo'),'<hp:renderingInfo><hc:transMatrix e1="1" e2="0" e3="0" e4="0" e5="1" e6="0"/><hc:scaMatrix e1="1" e2="0" e3="0" e4="0" e5="1" e6="0"/><hc:rotMatrix e1="1" e2="0" e3="0" e4="0" e5="1" e6="0"/></hp:renderingInfo>'));
 edits.push(patch(first(pic,'imgRect'),'<hp:imgRect><hc:pt0 x="0" y="0"/><hc:pt1 x="'+width+'" y="0"/><hc:pt2 x="'+width+'" y="'+height+'"/><hc:pt3 x="0" y="'+height+'"/></hp:imgRect>'));
 for(const p of children(first(tree.children[0],'subList'),'p')){if(!desc(p,'pic').length){if(clean(textOf(p,xml)))fail('공통 워터마크 바탕쪽에 예상하지 못한 글자가 있습니다.');edits.push(patch(p,''));}else{const lines=first(p,'linesegarray');edits.push(patch(lines,'<hp:linesegarray><hp:lineseg textpos="0" vertpos="0" vertsize="1000" textheight="1000" baseline="850" spacing="0" horzpos="0" horzsize="48192" flags="393216"/></hp:linesegarray>'));}}
 return edit(xml,edits);
}
...
function sealPlacement(page,sectionTree,headerXml,xml){
 ...
 for(const para of ps){const lines=children(first(para,'linesegarray'),'lineseg'),txt=textOf(para,xml).replace(/\s+$/,'');let fontHeight=0;
  ...
  // Match the report font ratios and spacing; the lower-right signature area
  // is clear even when the address/footnote occupies the left part of its line.
  let measured=0;for(const run of children(para,'run')){const cp=desc(head,'charPr').find(c=>attr(c,'id')===attr(run,'charPrIDRef')),ht=num(cp,'height')||1000,ra=first(cp,'ratio'),sp=first(cp,'spacing');for(const c of Array.from(textOf(run,xml))){const group=/[\uac00-\ud7af\u1100-\u11ff]/.test(c)?'hangul':'latin',ratio=attr(ra,group)===''?1:num(ra,group)/100;measured+=ht*((c.charCodeAt(0)<128?0.5:1)*ratio+num(sp,group)/100);}}
  const textWidth=Math.min(contentWidth,measured);const lx=alignment==='RIGHT'?contentWidth-textWidth:alignment==='CENTER'?(contentWidth-textWidth)/2:0;
 ...
 let lower=anchorY+142,upper=ch-mt-mb-alignShift-142;const capWidth=7087,ratio=7339/7612;
 ...
 for(const r of rects){if(r.para===page.signature||!r.txt)continue;if(r.right<=localLeft||r.left>=localRight)continue;const start=r.top-142,end=r.bottom+142;if(end<=center)lower=Math.max(lower,end);else if(start>=center)upper=Math.min(upper,start);else fail('서명란과 다른 문장이 겹칩니다. 원본 한글 서명칸의 여유 공간을 확인해 주세요.');}
...
async function inspect(input,options={}){
 ...
 for(const s of parts){for(const pic of desc(s.tree,'pic'))if(marker(pic,s.xml,SEAL_MARK))seals++;for(const page of s.pages){try{const found=desc(s.tree,'pic').filter(pic=>sealOwner(pic,s.xml)===attr(page.table,'id')),place=sealPlacement(page,s.tree,headerText,s.xml),expected=sealPicture(a.sealPicture,'0','0','source',place),expectedTree=parse(expected);if(found.length!==1||ancestors(found[0],'p')[0]!==place.anchor||ancestors(found[0],'tbl').length>0||found[0].start>page.table.start||!samePicture(found[0],s.xml,expectedTree.children[0],expected,true))valid=false;}catch(_){valid=false;}}}
...
async function normalize(input,options={}){
 ...
 for(const s of parts){const edits=[];for(const pic of desc(s.tree,'pic')){const img=first(pic,'img'),known=imageHashes.get(attr(img,'binaryItemIDRef'))===a.manifest.seal.sha256;if((known||marker(pic,s.xml,SEAL_MARK))&&(ancestors(pic,'tbl').some(t=>footerOf(t,s.xml))||s.pages.some(p=>sealOwner(pic,s.xml)===attr(p.table,'id'))||(marker(pic,s.xml,SEAL_MARK)&&s.pages.length===1&&!ancestors(pic,'tbl').length)))edits.push(patch(pic,''));}if(edits.length)zip.file(s.name,edit(s.xml,edits));}
 ...
 const foreignMasterIds=new Set(parts.filter(s=>!s.pages.length).flatMap(s=>desc(s.tree,'secPr').flatMap(prop=>children(prop,'masterPage').map(n=>attr(n,'idRef')))));
 if(masterRecords.some(m=>m.brandingOnly&&foreignMasterIds.has(attr(m.root,'id'))))fail('성적서 외 구역에서도 같은 워터마크 바탕쪽을 사용하고 있습니다. 다른 구역의 원본 보존을 위해 자동 위치 보정을 중단했습니다.');
 ...
 // A merged file has one branding master per source. Consolidate those exact,
 // branding-only masters so no stale inline watermark survives inspection.
 const obsolete=masterRecords.filter(m=>m.brandingOnly&&m.name!==masterPath&&!foreignMasterIds.has(attr(m.root,'id')));
 if(obsolete.length){const removePaths=new Set(obsolete.map(m=>m.name));hpf=edit(hpf,mi.items.filter(i=>removePaths.has(attr(i,'href'))).map(i=>patch(i,'')));for(const m of obsolete)zip.remove(m.name);mi=manifestInfo(hpf);}
 ...
 parts=await sectionParts(zip);for(const s of parts){if(!s.pages.length)continue;const edits=[];for(const p of s.pages){const place=sealPlacement(p,s.tree,styles.xml,s.xml),run=children(place.anchor,'run').slice(-1)[0];if(!run||Number(attr(first(p.cell,'cellSz'),'width'))<10000)fail('발행인 서명칸의 직인 위치를 확인해 주세요.');const seal=sealPicture(a.sealPicture,integerId(originalIds,2102000000),integerId(originalIds,2103000000),sealId,place);edits.push({start:p.table.start,end:p.table.start,text:seal});}

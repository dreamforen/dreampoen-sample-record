...
(function dfMeasurementReportHwpx(){
  ...
  function unitIssues(form){
    var issues=[];["fuel","product","incineration"].forEach(function(key){(form.operation[key]||[]).forEach(function(row,index){if(clean(row.amount)&&!clean(row.unit)&&!hasAnyUnit(row.amount))issues.push(({fuel:"연료사용량",product:"제품생산량",incineration:"소각량"})[key]+" "+(index+1)+"의 단위");});});
    (form.operation.material||[]).forEach(function(row,index){if(clean(row.amount)&&!clean(row.unit)&&!hasAnyUnit(row.amount))issues.push("원료투입량 "+(index+1)+"의 단위");});
    (form.results||[]).forEach(function(row,index){if(clean(row.item)&&!clean(row.unit)&&!resultUnit(row.item))issues.push("측정항목 "+(index+1)+"의 단위");});return issues;
  }
  ...
  function operationLines(rows){return (rows||[]).map(function(row){return withUnit(row.amount,row.unit,!!clean(row.unit));}).join("\n");}
  function mapTemplateValues(form){
    form=ensureArrays(form);var prevention=splitBalanced(form.sampling.prevention_rows,2),results=splitBalanced(form.results,2),issuer=form.issuer||{},date=isoDate(form.issue_date),dateKo=date?Number(date.slice(0,4))+" 년 "+Number(date.slice(5,7))+" 월 "+Number(date.slice(8,10))+" 일":"2026 년 00 월 00 일";
    var map={
  ...
  async function uploadFileRecord(report,blob,fileName,role,mime,formSnapshot){
    var db=database(),user=currentUser();if(!db||!user)throw new Error("온라인 DB에 로그인해주세요.");var version=await nextVersion(report.id),path="report-writer/reports/"+report.report_year+"/"+report.id+"/"+storageId()+"_"+safeName(fileName),upload=await db.storage.from(BUCKET).upload(path,blob,{contentType:mime||blob.type||"application/octet-stream",upsert:false});if(upload.error)throw upload.error;
  ...
  function previewForm(raw){
    ...
    ["fuel","product","incineration"].forEach(function(key){form.operation[key].forEach(function(row){row.amount=withUnit(row.amount,row.unit,!!clean(row.unit));});});

...
(function dfMeasurementReportHwpx(){
  ...
  function legacyCanUse(){var p=currentProfile(),role=clean(p&&p.role).toLowerCase(),a=p&&(p.access_permissions||p.board_permissions||{});return !!p&&(role==="admin"||role==="관리자"||a.repository!==false);}
  function allowed(action){var p=window.DFMenuPermissions;return p?p.can('measurement-reports',action,legacyCanUse):legacyCanUse();}
  ...
  async function loadData(force){
    ...
    try{
      ...
      if(settled[2].status!=="fulfilled")throw settled[2].reason;state.files=(settled[2].value||[]).filter(function(row){return !retiredFile(row);});
  ...
  function renderFolderList(){permissionPaint();
  ...
  function renderCompanyFolder(){permissionPaint();
    ...
    root.querySelectorAll("[data-rhx-brand-file]").forEach(function(b){b.onclick=function(){applyReportBranding([findFile(b.dataset.rhxBrandFile)],b);};});
    root.querySelectorAll("[data-rhx-archive-file]").forEach(function(b){b.onclick=function(){archiveFile(findFile(b.dataset.rhxArchiveFile));};});bindDrop();bindBulkDownloads(g);if(templateManager())templateManager().bindSection(root,g);
  }
  ...
  async function notifyHalfYear(report,completed){
    ...
    catch(error){diag("warn","반기보고자료 갱신 필요",error&&error.message||error);return {message:"성적서는 저장되었지만 반기보고자료 갱신에 실패했습니다. 반기보고서 메뉴에서 다시 확인해주세요.\n"+(error.message||error)};}
  }
  function reportCompletionIssues(report,form){
    ...
    rows.forEach(function(r){if(!clean(r.result))issues.push(r.item+": 측정값");if(!clean(r.unit))issues.push(r.item+": 단위");if(!clean(r.method))issues.push(r.item+": 측정방법");});
    return issues;
  }
  var completionBusy=false;
  async function completeReport(id,button){
    if(!askAction("update"))return;
    if(completionBusy)return;var db=database(),user=currentUser();if(!db||!user)return window.alert("온라인 DB에 로그인해주세요.");
    completionBusy=true;var label=button&&button.textContent;if(button){button.disabled=true;button.textContent="확인 중…";}
    try{
      var fetched=await db.from(REPORT_TABLE).select("*").eq("id",id).is("archived_at",null).maybeSingle();if(fetched.error)throw fetched.error;
      var report=fetched.data;if(!report)throw Error("성적서가 없거나 삭제되었습니다. 새로고침해주세요.");
      var fileResult=await db.from(FILE_TABLE).select("id,file_name,version_no").eq("report_id",id).is("archived_at",null).order("version_no",{ascending:false}).limit(1).maybeSingle();if(fileResult.error)throw fileResult.error;if(!fileResult.data)throw Error("성적서 HWPX를 생성하거나 최종 파일을 업로드한 뒤 작성완료해주세요.");
      ...
      if(report.status!=="issued"){
        ...
        var result=await query.select("*").maybeSingle();if(result.error)throw result.error;if(!result.data)throw Error("다른 곳에서 성적서가 수정되었습니다. 새로고침한 뒤 다시 확인해주세요.");report=result.data;
      }
      state.reports=state.reports.filter(function(r){return String(r.id)!==String(id);});state.reports.push(report);
      var half=await notifyHalfYear(report,true);renderCompanyFolder();window.alert("성적서 작성완료를 반영했습니다.\n"+clean(half&&half.message));
    }catch(error){window.alert("작성완료 처리 실패\n\n"+(error.message||error));}
    finally{completionBusy=false;if(button){button.disabled=false;button.textContent=label;}}
  }
  async function invalidateCompletedReport(report){
    if(!report||report.status!=="issued")return report;needAction("update");var db=database(),form=clone(report.form_data||{});delete form.completion_snapshot;
    var query=db.from(REPORT_TABLE).update({status:"draft",form_data:form,updated_by:currentUser()&&currentUser().id}).eq("id",report.id).is("archived_at",null);if(report.updated_at)query=query.eq("updated_at",report.updated_at);
    var result=await query.select("*").maybeSingle();if(result.error)throw result.error;if(!result.data)throw Error("성적서가 다른 곳에서 변경되었습니다. 새로고침 후 작성완료 상태를 다시 확인해주세요.");
  ...
  var preparedReportBlobs=new WeakSet();
  function brandingModule(){var module=window.DF_REPORT_BRANDING;if(!module||typeof module.normalize!=="function"||typeof module.inspect!=="function")throw Error("표준 워터마크·직인 구성요소를 불러오지 못했습니다. 새로고침 후 다시 시도해주세요.");return module;}
  async function prepareReportBlob(blob,name,mime){
    if(fileExtension(name)!==".hwpx"&&mime!=="application/hwp+zip")return blob;
    if(preparedReportBlobs.has(blob))return blob;
    var prepared=await brandingModule().normalize(blob);preparedReportBlobs.add(prepared);return prepared;
  }
  ...
  async function uploadFileRecord(report,blob,fileName,role,mime,formSnapshot,allocatedVersion){
    needAction("upload");
    blob=await prepareReportBlob(blob,fileName,mime);
  ...
  async function uploadRevisions(fileList,reportId){
    if(state.uploadBusy||!askAction("upload"))return;var report=state.reports.find(function(r){return String(r.id)===String(reportId);});if(!report)return window.alert("연결할 접수번호를 선택해주세요.");if(report.status==="issued"&&!askAction("update"))return;var files=Array.prototype.slice.call(fileList||[]).filter(function(file){return file&&file.name&&/\.(hwpx|hwp|pdf)$/i.test(file.name)&&Number(file.size)<=MAX_FILE_SIZE;});if(!files.length)return window.alert("HWPX, HWP 또는 PDF 파일을 선택해주세요. 파일당 최대 50MB입니다.");state.uploadBusy=true;
    try{for(var i=0;i<files.length;i++){var f=files[i],ext=(f.name.split(".").pop()||"").toLowerCase(),role=ext==="pdf"?"pdf":"revised";await uploadFileRecord(report,f,f.name,role,f.type||(ext==="hwpx"?"application/hwp+zip":"application/octet-stream"));}report=await invalidateCompletedReport(report);await loadData(true);window.alert("수정파일을 새 버전으로 저장했습니다. 이전 파일도 그대로 보관됩니다.\n최종 입력값을 확인한 뒤 작성완료·반기 반영을 눌러주세요.");}catch(error){window.alert("수정파일 업로드 실패\n\n"+migrationMessage(error));}finally{state.uploadBusy=false;var input=byId("rhxFileInput");if(input)input.value="";}
  }
  ...
  var downloadReadyBlobs=new WeakSet();
  async function prepareDownloadBlob(blob,file){
    if(fileExtension(file&&file.file_name)!==".hwpx"&&(!file||file.mime_type!=="application/hwp+zip"))return blob;
    if(downloadReadyBlobs.has(blob))return blob;
    var module=window.DF_REPORT_BRANDING;
    if(!module||typeof module.prepareDownload!=="function")throw Error("성적서 그림 확인 구성요소를 불러오지 못했습니다. 새로고침 후 다시 시도해주세요.");
    var ready=await module.prepareDownload(blob);downloadReadyBlobs.add(ready);return ready;
  }
  async function currentDownloadBlob(file){return prepareDownloadBlob(await storedBlob(file),file);}
  function latestBrandingCandidates(reports){return (reports||[]).map(function(report){return latestFile(report.id);}).filter(function(file){return file&&fileExtension(file.file_name)===".hwpx";});}
  async function assertLatestBrandingFile(file){
    var result=await database().from(FILE_TABLE).select("*").eq("report_id",file.report_id).is("archived_at",null).order("version_no",{ascending:false}).limit(1).maybeSingle();
    if(result.error)throw result.error;
    if(!result.data||String(result.data.id)!==String(file.id))throw Error("최신 버전이 변경되었습니다. 새로고침 후 최신 HWPX에 적용해주세요.");
    return result.data;
  }
  async function createStandardizedVersion(file){
    needAction("upload");
    if(!file||fileExtension(file.file_name)!==".hwpx")throw Error("HWPX 파일에 적용할 수 있습니다.");
    var report=reportForFile(file);if(!report)throw Error("연결된 성적서를 찾지 못했습니다.");
    file=await assertLatestBrandingFile(file);
    var blob=await storedBlob(file),module=brandingModule(),info=await module.inspect(blob);
    if(info.standardized)return null;
    var prepared=await prepareReportBlob(blob,file.file_name,"application/hwp+zip");
    await assertLatestBrandingFile(file);
    var version=await nextVersion(report.id),name=safeName(file.file_name.replace(/\.hwpx$/i,"").replace(/_표준직인_v\d+$/,"").replace(/_v\d+$/,""))+"_표준직인_v"+version+".hwpx";
    // Branding changes only: preserve the measured data and the issued report's completion snapshot.
    return uploadFileRecord(report,prepared,name,file.file_role||"revised","application/hwp+zip",file.form_snapshot?clone(file.form_snapshot):null,version);
  }
  async function applyReportBranding(files,button){
    if(state.uploadBusy)return;if(!files.length)return window.alert("최신 저장본이 HWPX인 성적서가 없습니다.");
    state.uploadBusy=true;var saved=0,skipped=0,failed=[],label=button&&button.textContent;
    if(button)button.disabled=true;
    try{for(var i=0;i<files.length;i++){if(button)button.textContent="적용 중 "+(i+1)+" / "+files.length;try{if(await createStandardizedVersion(files[i]))saved++;else skipped++;}catch(error){failed.push(files[i].file_name+": "+(error.message||error));}}
      render();window.alert("표준 워터마크·직인 적용본 "+saved+"건을 새 버전으로 저장했습니다.\n이미 적용된 파일 "+skipped+"건"+(failed.length?"\n\n적용하지 못한 파일:\n"+failed.join("\n"):""));
    }finally{state.uploadBusy=false;if(button){button.disabled=false;button.textContent=label;}}
  }
  function retired(value){var api=window.DF_RECEIPT_RETIREMENT;return !!(api&&api.isRetired(value));}
  function retiredFile(file){var report=reportForFile(file);return retired(report||file);}
  function bulkCandidates(group,allVersions){
    var reports=(group&&group.reports||[]).filter(function(r){return !r.archived_at&&!retired(r);}),byId=new Map(reports.map(function(r){return [String(r.id),r];}));
    var files=(group&&group.files||[]).filter(function(f){return !f.archived_at&&fileExtension(f.file_name)===".hwpx"&&byId.has(String(f.report_id))&&!retired(byId.get(String(f.report_id)));});
    files.sort(function(a,b){var ar=byId.get(String(a.report_id)),br=byId.get(String(b.report_id));return clean(br.updated_at).localeCompare(clean(ar.updated_at))||Number(b.version_no||0)-Number(a.version_no||0)||clean(b.updated_at||b.created_at).localeCompare(clean(a.updated_at||a.created_at));});
    var seen=new Set();return files.filter(function(f){var r=byId.get(String(f.report_id)),key=clean(r.source_receipt_no||r.report_no)||String(r.id);if(!allVersions&&seen.has(key))return false;seen.add(key);return true;}).sort(function(a,b){var ar=byId.get(String(a.report_id)),br=byId.get(String(b.report_id)),af=clean(ar.facility_name||ar.form_data&&ar.form_data.request&&ar.form_data.request.stack_name),bf=clean(br.facility_name||br.form_data&&br.form_data.request&&br.form_data.request.stack_name);return af.localeCompare(bf,'ko',{numeric:true})||clean(ar.measurement_date).localeCompare(clean(br.measurement_date))||clean(ar.source_receipt_no||ar.report_no).localeCompare(clean(br.source_receipt_no||br.report_no),'ko',{numeric:true})||Number(a.version_no||0)-Number(b.version_no||0)||clean(a.id).localeCompare(clean(b.id));});
  }
  function bulkFileName(file,parent){var report=parent||reportForFile(file)||{},receipt=clean(report.source_receipt_no||report.report_no||file.source_receipt_no)||"접수번호미등록",facility=clean(report.facility_name||report.form_data&&report.form_data.request&&report.form_data.request.stack_name),parts=[receipt,facility,file.file_name];return parts.filter(Boolean).join('_');}
  function bindBulkDownloads(group){var button=byId('rhxBulkDownload'),scope=byId('rhxBulkScope'),status=byId('rhxBulkStatus');if(!button||!scope)return;function count(){var n=bulkCandidates(group,scope.value==='all').length;button.disabled=state.bulkBusy||!n;scope.disabled=state.bulkBusy;if(status){var available=new Set(bulkCandidates(group,true).map(function(f){return String(f.report_id);})),missing=(group.reports||[]).filter(function(r){return !r.archived_at&&!retired(r)&&!available.has(String(r.id));}).length;status.textContent=n+'개 HWPX'+(missing?' · HWPX 없는 접수 '+missing+'건':'');}}scope.onchange=count;button.onclick=function(){downloadCompanyHwpx(group,scope.value==='all',button,status);};count();}
  async function downloadCompanyHwpx(group,allVersions,button,status){
    if(state.bulkBusy)return;state.bulkBusy=true;var label=button&&button.textContent,scope=byId('rhxBulkScope');if(button)button.disabled=true;if(scope)scope.disabled=true;
    try{needAction('view');var downloadYear=state.year,companyName=group.name,reportSnapshot=new Map((group.reports||[]).map(function(r){return [String(r.id),clone(r)];})),files=bulkCandidates(group,allVersions).map(function(f){return Object.assign({},f);}),merger=window.DF_REPORT_MERGE;if(!merger)throw Error('한글 합본 구성요소를 불러오지 못했습니다. 새로고침해주세요.');var blob=await merger.create({files:files,title:companyName+' '+downloadYear+' 성적서 합본',load:function(file){needAction('view');if(retired(reportSnapshot.get(String(file.report_id))||file))throw Error('삭제된 접수자료입니다. 목록을 새로고침해주세요.');return currentDownloadBlob(file);},progress:function(done,total){if(button)button.textContent=done===total?'한글 합본 구성 중…':'받는 중 '+done+' / '+total;if(status)status.textContent=done+' / '+total+'개';}});downloadBlob(blob,safeName(companyName)+'_'+downloadYear+'_성적서합본_'+(allVersions?'모든버전':'접수별최신')+'.hwpx');if(status)status.textContent=files.length+'개 성적서 · HWPX 1개 다운로드 완료';}
    catch(error){if(status)status.textContent='다운로드 중단';window.alert(error.message||error);}
    finally{state.bulkBusy=false;if(button){button.disabled=false;button.textContent=label;}if(scope)scope.disabled=false;if(button!==byId('rhxBulkDownload')){var activeGroup=selectedGroup();if(activeGroup)bindBulkDownloads(activeGroup);}}
  }
  ...
  async function openHwpxImagePreview(file,blob,autoPrint){
    var preview=window.DF_HWPX_PREVIEW;if(!preview||typeof preview.render!=="function")throw Error("HWPX 미리보기 구성요소를 불러오지 못했습니다. 새로고침 후 다시 시도해주세요.");
    blob=await prepareDownloadBlob(blob,file);
    var view=await preview.render(blob,{name:file.file_name});revokePreview();state.previewView=view;
    var old=byId("rhxPreviewModal");if(old)old.remove();
    var modal=document.createElement("div");modal.id="rhxPreviewModal";modal.className="rhx-preview-modal";modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');modal.setAttribute('aria-label',file.file_name);modal.setAttribute('data-df-modal-root','');
    modal.innerHTML='<div class="rhx-preview-card"><header><div><b>'+esc(file.file_name)+'</b><small>저장된 HWPX의 실제 본문과 표를 표시합니다.</small></div><nav><button class="rhx-btn" id="rhxPreviewPrint">인쇄</button><button class="rhx-btn" id="rhxPreviewDownload">HWPX 다운로드</button><button class="rhx-btn" id="rhxPreviewClose">닫기</button></nav></header><div class="rhx-preview-body"></div></div>';
    modal.querySelector('.rhx-preview-body').appendChild(view.element);document.body.appendChild(modal);
    byId("rhxPreviewClose").onclick=function(){modal.remove();revokePreview();};byId("rhxPreviewDownload").onclick=function(){downloadBlob(blob,file.file_name);};byId("rhxPreviewPrint").onclick=function(){Promise.resolve(view.print()).catch(function(e){window.alert(e.message||e);});};
    if(autoPrint)Promise.resolve(view.print()).catch(function(e){window.alert(e.message||e);});
  }
  function revokePreview(){if(state.previewView){state.previewView.destroy();state.previewView=null;}if(state.previewUrl){try{URL.revokeObjectURL(state.previewUrl);}catch(ignore){}state.previewUrl="";}}
  var archiveBusy=new Set();
  async function archiveFile(file){
    if(!askAction("delete"))return;

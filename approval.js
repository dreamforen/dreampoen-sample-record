...
(function(){
  ...
  async function startForm(preset=null){if(!db()||!me())return alert('로그인 후 기안할 수 있습니다.');if(!state.profiles.length){await load();if(!state.profiles.length)return}openForm(null,preset)}
  function openForm(existing=null,preset=null){
    ...
    const addFiles=files=>{if(editor.busy)return;for(const f of files){if(f.size>20*1024*1024){alert(f.name+' 파일은 20MB 이하만 가능합니다.');continue}if(!editor.files.some(x=>x.file.name===f.name&&x.file.size===f.size&&x.file.lastModified===f.lastModified))editor.files.push({file:f,id:crypto.randomUUID(),done:false})}drawFiles()};

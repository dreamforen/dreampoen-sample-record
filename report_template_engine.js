...
(function (global) {
  ...
  function analyte(v){var s=norm(v).replace(/mg\/?s?[㎥³3]|ppm|ppb|%/g,'');if(/총탄화수소|totalhydrocarbon|^thc$/.test(s))return '총탄화수소';return s.replace(/thc/g,'');}
  ...
  function inspectPage(p,sectionName,paragraphIndex,xml,setup){
    ...
    var resultHead=unique(byLabel(cells,'측정항목'),'result-header'),period=byLabel(cells,'분석기간')[0],resultRows=[];
    if(resultHead&&period){
      ...
      var known={};resultRows.forEach(function(item){var name=clean(item.value).replace(/\([^)]*(?:ppm|ppb|mg|㎥|%)\)/gi,'').trim(),a=analyte(name),rowCells=cells.filter(function(c){return c.row>=item.row&&c.row<item.row+item.rowSpan&&c.col>item.col;});
        if(!name&&!rowCells.some(function(c){return clean(c.value)&&clean(c.value)!=='~';}))return;
        if(!a||/\$/.test(a)){issues.push(issue('unknown-analyte:'+item.cellId,'측정항목 이름이 없는 결과 행에 값이 있습니다. 한글 원본에 정확한 항목명을 입력한 뒤 다시 등록해 주세요.'));a='';}
        if(a&&known[a])issues.push(issue('duplicate-analyte:'+a,'원본에 같은 측정항목이 중복되어 있습니다: '+name));known[a]=true;
        ['limit','result','time','method','memo'].forEach(function(key){var h=heads[key];if(!h)return;var candidates=rowCells.filter(function(c){return c.col>=h.col&&c.col<h.col+h.colSpan;});
          if(key==='time'){var ts=candidates.filter(function(c){return clean(c.value)!=='~'&&!(c.col!==h.col&&!clean(c.value));}).sort(function(a,b){return a.row-b.row||a.col-b.col;});if(ts.length===2){add('result.time',ts[0],{analyte:name,part:'start'});add('result.time',ts[1],{analyte:name,part:'end'});}else if(ts.length===1){add('result.time',ts[0],{analyte:name,part:'range'});}else issues.push(issue('result-time:'+item.cellId,'측정시간 칸을 확정하지 못했습니다: '+name));
  ...
  async function generate(bytes,config,form){
    ...
    var available={};config.mapping.filter(function(m){return m.field==='result.result';}).forEach(function(m){available[analyte(m.analyte)]=true;});
    (form.results||[]).forEach(function(r){if(clean(r.item||r.name)&&!available[analyte(r.item||r.name)])fail('원본 양식에 없는 측정항목입니다: '+(r.item||r.name)+'. 해당 항목이 있는 원본을 등록하거나 결과 칸을 직접 연결해 주세요.');});
    config.mapping.forEach(function(m){patches=patches.concat(patchCell(cellMap[m.cellId],m,valueFor(m,form),xml));});
  ...
  global.DF_REPORT_TEMPLATE_ENGINE={version:VERSION,inspect:inspect,validateConfig:validateConfig,generate:generate,fieldOptions:OPTIONS,_test:{parse:parse,applyPatches:applyPatches,analyte:analyte,formatted:formatted}};

(function(){'use strict';
 ...
 const S={sources:[],catalog:[],source:null,row:null,payload:null,history:[],kind:'manual',mode:'edit',range:null,loading:false,composing:false,disposed:false,previewValid:false,overflow:false,identity:null,followCurrent:true,actionBusy:false,syncing:null};
 const STATUS={source:'검토 기준자료 · 미승인',draft:'개정안 작성 중 · 미승인',submitted:'개정안 결재 중 · 미승인',active:'승인본',obsolete:'구본',rejected:'반려',cancelled:'개정 취소 · 미시행'};
 const REV_FIELDS='id,doc_key,revision,status,approval_id,effective_date,reason,lock_version,created_by,created_at,updated_at';
 ...
 const writable=()=>!!(S.row?.status==='draft'&&!S.row.approval_id&&!S.approvalPending&&can('update')&&!S.disposed);
 ...
 const queue=new C.SaveQueue({
  ...
  onSaved(row){S.row=row;S.catalog=S.catalog.filter(x=>x.id!==row.id);S.catalog.push(row);updateMeta();renderToc();}
 });
 ...
 function applyToc(area){if(liveToc())return C.bindToc(area,Object.fromEntries(S.catalog.filter(r=>r.status==='active').map(r=>[r.doc_key,r])));return 0;}
 function updateMeta(){
  if(!S.source)return;const m=metadata(),active=activeFor(m.key),official=active?'시행 '+rev(active.revision):'기준 '+rev(S.source.base_revision)+' (미승인)',working=['draft','submitted'].includes(m.status);
  $('code').textContent=m.key+' · '+official+(working?' · 개정안 '+rev(m.revision):!S.followCurrent?' · 이력 '+rev(m.revision):'');$('title').textContent=S.source.title;
  $('meta').textContent=STATUS[m.status]+(S.row?.approval_id?' · 결재문서 연결됨':'')+(m.status==='cancelled'?' · '+(S.row.cancel_reason||'취소 이력 보존'):m.effectiveDate?' · 시행일 '+m.effectiveDate:' · 시행일은 승인 시 반영');
  $('modeStatus').textContent=STATUS[m.status]+' · 시행 번호는 최종 승인 후 반영 · 취소 시 번호 유지';
  ...
  $('approve').disabled=locked||!B; $('upload').disabled=S.actionBusy||S.loading||!can('upload')||!!(S.row&&!editable());
  ...
  $('tocNotice').hidden=!liveToc();$('tocNotice').textContent='목차의 개정번호는 현재 승인본을 자동 반영합니다. 원본과 이전 개정본의 목차는 개정이력에서 확인하세요.';
 ...
 function renderToc(){
  ...
  $('toc').innerHTML=items.map(d=>{const a=activeFor(d.doc_key),w=workingFor(d.doc_key);return `<button type="button" data-key="${esc(d.doc_key)}" class="${d.doc_key===S.source?.doc_key?'active':''}"><small>${esc(d.doc_key)}</small>${esc(d.title)}<span class="toc-revision">${a?'시행 '+rev(a.revision):'기준 '+rev(d.base_revision)+' · 미승인'}</span>${w?`<span class="toc-working">개정안 ${rev(w.revision)} · ${w.status==='submitted'?'결재 중':w.approval_id?'기안 연결됨':'작성 중'}</span>`:''}</button>`;}).join('')||'<p>등록된 문서가 없습니다.</p>';
 ...
 async function refresh(){
  ...
  ]);S.sources=sources;S.catalog=catalog;
  renderToc();
 }
 ...
 async function openDocument(key,revisionId){
  ...
  try{
   ...
   S.source=src;S.kind=src.kind;S.row=selected;S.history=revisions;S.payload=C.payload(selected?.payload||src.payload);S.range=null;S.previewValid=false;S.approvalPending=false;S.followCurrent=!revisionId;
 ...
 function selection(){if(!editable()||S.loading||S.mode!=='edit')throw Error('개정 초안의 편집 모드에서 사용하세요.');if(!S.range||!S.range.startContainer.isConnected)throw Error('본문에서 적용할 위치나 글자를 먼저 선택하세요.');const s=window.getSelection();s.removeAllRanges();s.addRange(S.range);return s;}
 ...
 async function approval(){
  ...
  try{B.approval({doc_type:'general',locked:true,title:`${S.source.doc_key} ${S.source.title} Rev.${String(S.row.revision).padStart(2,'0')} 개정 승인`,content:`DF-QW:${S.row.id}:${S.row.lock_version}\n[첫 줄은 문서 연결정보이므로 수정하지 마세요.]\n문서번호: ${S.source.doc_key}\n개정번호: ${S.row.revision}\n개정사유: ${S.row.reason}\n\n품질문서 작업실에서 연결된 저장본을 검토하세요. 원본 대조 확인: 완료\n이 연결 이후의 내용 변경은 잠기며, 기존 결재선의 최종 승인 후에만 시행됩니다.`});}
 ...
 async function selectKind(kind){
  ...
  try{
   ...
   const first=S.sources.find(x=>x.kind===kind);if(first){await openDocument(first.doc_key);return;}
   ...
   for(const id of ['save','newDraft','approve','cancelRevision','print','upload','reviewed'])$(id).disabled=true;
 ...
 async function syncStatus(force=false){
  ...
  S.syncing=(async()=>{
   const id=S.row?.id,key=S.source?.doc_key,stamp=catalogStamp(S.catalog);
   const [sources,rows,selected]=await Promise.all([
    result(api().from('quality_workspace_sources').select('doc_key,kind,title,base_revision,source_sha256').order('doc_key')),
    result(api().from('quality_workspace_revisions').select(REV_FIELDS).in('status',['active','draft','submitted'])),
   ...
   S.sources=sources;S.catalog=rows;renderToc();
   ...
   if(changed||switched){
    if(queue.dirty()||S.composing){
     if(changed){clearTimeout(queue.timer);queue.error=Error('QW_CONFLICT: 서버 저장본 또는 결재 상태가 변경되었습니다. 입력을 백업한 뒤 새로고침하세요.');queue.onState('error',queue.error);}
     updateMeta();return;
    }
    if(key)await openDocument(key,S.followCurrent?undefined:id||'source');
   }else{
    if(S.approvalPending&&!B.approvalBusy?.()){S.approvalPending=false;say('저장된 결재문서 연결이 없습니다. 본문 편집을 계속할 수 있습니다.',true);}
    updateMeta();
   }
   if(liveToc()&&stamp!==catalogStamp(rows)&&!S.composing){
 ...
 function permissionChanged(){
  ...
  if(identityChanged||revoked){
   ...
   for(const id of ['save','newDraft','approve','cancelRevision','print','upload','reviewed'])$(id).disabled=true;$('formatTools').disabled=true;$('paragraphTools').disabled=true;
 ...
 window.DFQualityWorkspace={flush,isPending:()=>queue.dirty()||!!queue.running||S.actionBusy,permissionChanged,selectKind,sync:()=>syncStatus(true),dispose};

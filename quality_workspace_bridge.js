...
(function(){'use strict';
 ...
 async function leave(){
  const ws=workspace();if(ws){if(ws.isPending?.())await ws.flush();if(ws.isPending?.())throw Error('개정 처리가 끝난 뒤 이동하세요.');ws.dispose?.();}
  ...
  hub=null;if(returnView==='quality-manual')window.v62ShowOnly?.('quality-manual');
 }
 async function open(from,kind){
  kind=kind||['manual','procedure','instruction'].find(k=>can(k,'view'));
  if(!kind||!can(kind,'view'))return alert('품질문서 조회 권한을 확인하세요.');
  if(frame){window.v62ShowOnly?.(returnView);await workspace()?.selectKind?.(kind);notify();frame.focus();return;}
  ...
  frame=document.createElement('iframe');frame.title='품질문서 전체화면 작업실';frame.src='quality_workspace.html?v=4&kind='+encodeURIComponent(kind);
 ...
 const launch=(from,kind)=>open(from,kind).catch(e=>alert(e.message||String(e)));
 window.DFQualityBridge=Object.freeze({context,can,leave,approvalBusy,
  approval(preset){
 ...
 function mount(){
  ...
  if(target&&!entry){entry=document.createElement('button');entry.type='button';entry.className='company-btn primary';entry.textContent='품질문서 작업실 · 문서형 편집';entry.style.cssText='margin:14px 0;padding:12px 18px;';entry.addEventListener('click',()=>launch('quality'));target.prepend(entry);}
  if(actions&&!actions.querySelector('[data-qw-open]')){const b=document.createElement('button');b.type='button';b.dataset.qwOpen='1';b.className='company-btn secondary';b.textContent='2025 검토본 · 문서형 작업실';b.addEventListener('click',()=>launch('quality-manual','manual'));actions.append(b);}
  if(!document.getElementById('dfQualityWorkspaceLegacyStyle')){
   ...
   style.textContent='#dfQmsSectionMode,#dfViewQualityManual .df-qms-mode-tabs,#dfQmsSummary{display:none!important}';document.head.append(style);
 ...
 document.addEventListener('click',e=>{
  const category=e.target.closest?.('[data-doc-category="quality_procedure"],[data-doc-category="quality_instruction"]');
  if(category){e.preventDefault();e.stopImmediatePropagation();launch('quality',category.dataset.docCategory==='quality_procedure'?'procedure':'instruction');return;}

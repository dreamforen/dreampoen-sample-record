...
function calcCore(){
  ...
  const o2=o2Average(), co2=num('#co2');
...
function recalc(){
  ...
  $('#rMoist').textContent=fmt(c.moist,1); $('#rDensity').textContent=fmt(c.density,4); $('#rVelocity').textContent=fmt(c.velocity,3);
  $('#rArea').textContent=fmt(c.area,4); $('#rFlow').textContent=fmt(c.flow,3); $('#rIso').textContent=fmt(iso,2);
}
...
function collect(){
  const obj={recordType,selectedTeam,fields:{},moist:$$('.moist').map(x=>x.value),o2vals:$$('.o2val').map(x=>x.value),points:[]};
...
$('#btnSave').onclick=()=>{localStorage.setItem('dreampoen_sample_record_v2',JSON.stringify(collect())); $('#saveStatus').textContent='이 브라우저에 임시저장 완료 · '+new Date().toLocaleString()};
$('#btnLoad').onclick=()=>{const s=localStorage.getItem('dreampoen_sample_record_v2')||localStorage.getItem('dreampoen_sample_record_v1'); if(!s)return alert('저장된 기록이 없습니다.'); apply(JSON.parse(s)); $('#saveStatus').textContent='저장된 기록을 불러왔습니다.'};
$('#btnReset').onclick=()=>{if(confirm('현재 입력값을 초기화할까요?')){localStorage.removeItem('dreampoen_sample_record_v2');location.reload()}};
$('#btnPrint').onclick=()=>window.print();
$('#btnExcel').onclick=()=>{
  if(typeof XLSX==='undefined')return alert('Excel 라이브러리를 불러오지 못했습니다. 인터넷 연결을 확인해주세요.');
  const o=collect(), c=calcCore();
  const rows=[['대기 시료채취기록지'],['측정구분',recordType==='dust'?'먼지':'중금속'],['접수번호',o.fields.receiptNo],['측정날짜',o.fields.measureDate],['업체명',o.fields.company],['시설명',o.fields.facility],['측정팀',selectedTeam+'팀'],['오리피스계수',orificeCoeff()],['노즐직경(cm)',o.fields.nozzleCm],[],['기상',o.fields.weather,'기온',o.fields.airTemp,'습도',o.fields.humidity,'측정위치 대기압',o.fields.locationPressure,'대기압',o.fields.pressure,'풍향',o.fields.windDir,'풍속',o.fields.windSpeed],['산소평균(%)',$('#o2Avg').textContent,'수분평균(%)',$('#moistAvg').textContent],[],['포인트','채취시간(min)','가스온도(℃)','정압(mmH2O)','동압(mmH2O)','오리피스압차(mmH2O)','진공게이지압(mmHg)','홀더온도(℃)','미터In(℃)','미터Out(℃)','임핀저출구(℃)','채취량(L)']];
